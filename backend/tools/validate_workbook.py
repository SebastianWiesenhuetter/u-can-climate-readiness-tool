...
# REQ_LONG = {"question_id", "value"}  # respondent_ref and created_at optional

# def guess_city_id(sheet):
#     # numeric id allowed, or label in CITY_MAP, or id exactly equals the mapped id
#     if sheet in CITY_MAP: return CITY_MAP[sheet]
#     if sheet in CITY_MAP.values(): return sheet
#     try:
#         int(sheet); return sheet
#     except: pass
#     return None
...
#     wide_cols = [c for c in cols if c.startswith("q_")]
#     if wide_cols:
#         needs_resp = "respondent_ref" in cols
#         return "wide", needs_resp, cols
#     if REQ_LONG.issubset(cols):
#         return "long", True, cols
#     return "unknown", False, cols

# def main(path):
#     xls = pd.ExcelFile(path)
#     print(f"Workbook: {path}")
#     for sheet in xls.sheet_names:
#         city = guess_city_id(sheet)
#         df = xls.parse(sheet)
#         n = len(df)
#         fmt, ok, cols = detect_format(df)
#         msg = []
#         if city is None:
#             msg.append("no-city-mapping")
#         if fmt == "wide" and "respondent_ref" not in cols:
#             msg.append("wide-format-needs-respondent_ref")
#         if fmt == "unknown":
#             missing = ", ".join(sorted(REQ_LONG - cols))
#             msg.append(f"missing-cols:{missing or '??'}")
#         print(f"- Sheet '{sheet}': rows={n}, city={city or '—'}, format={fmt}{' ✅' if fmt!='unknown' and (city or fmt=='unknown') else ''} {('[' + ', '.join(msg) + ']') if msg else ''}")
...
#     if len(sys.argv) != 2:
#         print("Usage: python tools/validate_workbook.py /path/to/workbook.xlsx")
#         sys.exit(1)
#     main(sys.argv[1])
